...
class ParkingCalculator {
    ...
    getFormattedReport() {
        ...
        if (this.input.areaNNP && this.input.areaNNP > 0) {
            report += `
        ...
        if ((this.input.areaFlats && this.input.areaFlats > 0) || (this.input.areaNNP && this.input.areaNNP > 0)) {
            report += `
            ...
            <table border="1" cellpadding="8">
                ...
                ${this.input.areaFlats && this.input.areaFlats > 0 ? `<tr><td>Жилое назначение</td><td>кв.м на одно место остановки</td><td>22100</td><td>${this.results.Nk_residential} (шт.)</td></tr>` : ''}
                ${this.input.areaNNP && this.input.areaNNP > 0 ? `<tr><td>Встроенно-пристроенные помещения коммерческого использования на первых этажах</td><td>кв.м на одно место остановки</td><td>450</td><td>${this.results.Nk_commercial} (шт.) ${this.input.areaNNP && this.input.areaNNP < 450 ? '- при площади коммерции менее 450 кв.м места остановки можно не предусматривать' : '- не далее 150 метров от входной группы (требуется не менее 1м/м и не более 4 м/м)'}</td></tr>` : ''}
